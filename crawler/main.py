...
def course(conn, cursor):
    ...
    merged_df = pd.merge(course_df, dept_df, left_on="授課對象", right_on='部門名稱',  how="left")
...
def section(conn, cursor):
    ...
    merged_df = pd.merge(section_df, prof_df, how='left', left_on='授課教師', right_on='姓名')
    merged_df = merged_df[['流水號', '學年-學期', '時間', '教室', '總人數', '課號', 'id']]
    merged_df['總人數'] = merged_df['總人數'].astype(int)
    merged_df['時間'] = merged_df['時間'].apply(parse_time)
    merged_df['時間'] = merged_df['時間'].apply(lambda x: json.dumps(x, ensure_ascii=False))
...
def syllabus(conn, cursor):
    ...
    merged_df = pd.merge(filtered_df, professor_df, how='left', left_on='授課教師', right_on='姓名')
    selected_columns = ['課程概述', '課程目標', '課程要求', '預期每週課後學習時數', 'Office Hours', '指定閱讀', '參考書目', 'id', '流水號']
    ...
    df['section_id'] = df['semester'].str.replace('-', '') + df['section_id'].astype(str)
    merged_df = pd.merge(merged_df, df, how='left', left_on='流水號', right_on='section_id')
    selected_columns = ['課程概述', '課程目標', '課程要求', '預期每週課後學習時數', 'Office Hours', '指定閱讀', '參考書目', 'id', 'ID']
