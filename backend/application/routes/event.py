...
def create():
    try:
        ...
        post_path, post_message, post_semester, post_section, post_courseName = data["path"], data["message"], data["semester"], data["section"], data["courseName"]
...
def participate():
    try:
        ...
        part_event = ParticipateEvent.query.filter(
            db.and_(ParticipateEvent.student_id == current_user.id, ParticipateEvent.event_id == event_id)
        ).first()
...
def result(eventid):
    try:
        ...
        part_event = ParticipateEvent.query.filter(ParticipateEvent.event_id == eventid).all()
