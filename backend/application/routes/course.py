...
from application.models import Section, Course, Department, IsTA, User
...
def create():
    try:
        ...
        course = Course(
            ...
            credits=credits,
            type=ctype,
            department_id=current_user.getDepartment.id,
        )
...
@course_route.route("/check", methods=["GET"])
def check():
    try:
        data = request.args.to_dict()
        courseId = data["courseId"]
        professorId = data["professorId"]
        prof = User.query.filter(User.id == professorId).first()
            ...
            jsonify(
                {
                    ...
                    "type": course.type,
                    "professor": prof.name,
                    "department": course.getDepartment.trivial_name,
                }
            ),
            200,
        )
